...
void spikeE(const int whichHit, Vector<double>& Clock, vector<int>& VE,
            ...
            uniform_real_distribution<double>& u, int &E_spike) {
  (*E_spike)++;
...
void spikeI(const int whichHit, Vector<double>& Clock, vector<int>& VI,
            ...
            vector<int>& awakeE, vector<int>& awakeI, mt19937& mt,
            uniform_real_distribution<double>& u, int &I_spike) {
  (*I_spike)++;
  VI[whichHit] = 0;
  awakeI[whichHit] = 0;
...
void simulate(double kickE, double kickI, double SEE, double SIE, double SEI,
              double SII, double terminate_time, int thread_id) {
    time_t seconds = time(NULL);

  random_device rd;
  mt19937 mt(rd(seconds + thread_id));
...
double* parallel_random(vector<double> &randomArr, int numRandoms, double from, double to) {
...
int main(int argc, char* argv[]) {
  ...
  int numRandoms = 10000;
    
//  vector<vector<int> > Rates_Sample(numRandoms, vector<int>(6));
    

    
    vector<double> SEEArr(numRandoms);
    vector<double> SEIArr(numRandoms);
    vector<double> SIEArr(numRandoms);
    vector<double> SIIArr(numRandoms);
  ...
  parallel_random(SEEArr, numRandoms, SEE_low, SEE_high);
  parallel_random(SEIArr, numRandoms, SEI_low, SEI_high);
  parallel_random(SIEArr, numRandoms, SIE_low, SIE_high);
  parallel_random(SIIArr, numRandoms, SII_low, SII_high);
...
#pragma omp parallel for
  for (int i = 0; i < numRandoms; i++) {
    ...
    double E_rate, I_rate = simulate(kickE, kickI, SEEArr[i], SIEArr[i], SEIArr[i], SIIArr[i], terminate_time, rank);

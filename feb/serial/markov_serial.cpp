...
void simulate(double kickE, double kickI, double terminate_time, ofstream& myfile)
{
    random_device rd;
    mt19937 mt(rd());
...
double uniform_range(double range_from, double range_to){
    random_device rd;
    mt19937 mt(rd());
    uniform_real_distribution<double> u(range_from, range_to);
...
//    if (argc != 5){
//        cout << "Must pass in SEE, SIE, SEI, SII e.g. ./markov_mod.out 5 4 -3 -3" << endl;
//        exit(-1);
//    }
//
...
//    SII = atof(argv[4]);
//
//    cout << "SEE, SIE, SEI, SEE " << SEE << " " << SIE << " " << SEI << " " << SII << endl << endl;
//
//    // SQE must be positive while SQI must be negative
//    if (SEE < 0 || SIE < 0 || SEI > 0 || SII > 0){
//        throw invalid_argument("SEE, SIE must be positive while SEI, SII must be negative");
//    }
    ...
    for (int i=0; i < 30; i++)
    {
        ...
        SEE = uniform_range(SEE_low, SEE_high);
        SIE = uniform_range(SIE_low, SIE_high);
        SEI = uniform_range(SEI_low, SEI_high);
        SII = uniform_range(SII_low, SII_high);

        
        myfile<<"SEE, SIE, SEI, SII: " << SEE<<" "<<SIE<<" "<<SEI<<" "<<SII<<" " << endl;
        kickE = 1000 + i*2000;
        kickI = kickE;
        myfile << "kichE, kichI: " << kickE<< " " << kickI << " " << endl;
        
        
        cout << "Simulation for kickE: " << kickE << ", kichI: " << kickI << endl;
        simulate(kickE, kickI, terminate_time, myfile);
        cout << endl;

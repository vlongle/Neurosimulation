...
void update(vector<int> &result, double HitE, mt19937& mt, uniform_real_distribution<double>& u,  Vector<double>& Clock,
...
void simulate(int thread_num, vector<int> &result, double hitE){
...
void get_random(double* randNumbers, double low, double high, int N){
    ...
    // format X, y \n
    // hitIs, y \n
    void write_to_disk(vector<vector<int> > &results, double* hitIs, int N){
        char* file_name = "varyHitE_2000.txt";
        ofstream myfile;
    ...
    void generateData(vector<vector<int> > &results, double hitEs[]){
    ...
    write_to_disk(results, hitEs, NUM_SAMPLES);

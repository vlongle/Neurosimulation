...
int Reverse = -66;
double kickE = 7000.0; // lambda_E
double kickI = 7000.0; // lambda_I
...
void spikeE(const int whichHit, Vector<double>& Clock, vector<int> &VE, Vector<int> &HEE, Vector<int> &HIE, Vector<int> &Eref, vector<int> &awakeE, vector<int> &awakeI, mt19937& mt, uniform_real_distribution<double>& u, double HitE)
{
    VE[whichHit] = 0;
    awakeE[whichHit] = 0;
...
void update(vector<int> &result, vector<double> &time_spike, vector<int> &num_spike, double HitE, mt19937& mt, uniform_real_distribution<double>& u,  Vector<double>& Clock,
        ...
        switch (index)
        {
            case 0:
                ...
                if(awakeE[whichHit])
                {
                    ...
                    {
                        spikeE(whichHit, Clock, VE, HEE, HIE, Eref, awakeE, awakeI, mt, u, HitE);
                         result[bin_num] ++;
...
void simulate(int thread_num, vector<int> &result, vector<double> &time_spike, vector<int> &num_spike, double HitE){
    ...
    update(result, time_spike, num_spike, HitE, mt, u,  Clock,
...
void write_to_disk(vector<vector<int> > &results, vector<vector<int> > &num_spike,
                   vector<vector<double> > &time_spike, double* hitEs, int N){
        char* file_name = "./dataHitE/varyHitE_2000.txt";
        ofstream myfile;
    ...
    char generic_file[] = "varyHitE_init_.txt";
    string base(".txt");
...
//            temp_str = strcat(strcat(generic_file, temp_str), ".txt");
            ...
            myfile.open("./dataHitE/varyHitE" + to_string(k) + base );
    ...
    void generateData(vector<vector<int> > &results, vector<vector<int> > &num_spike,
                      vector<vector<double> > &time_spike, double hitEs[]){
        ...
        for(int i=0; i < NUM_SAMPLES; i++){
            ...
            simulate(thread_num, res_arr, time_spike_arr, num_spike_arr, hitE);
    ...
    int main()
    {
        ...
        double hitEs[NUM_SAMPLES];
        get_random(hitEs, low, high, NUM_SAMPLES);
        ...
        generateData(results, num_spike, time_spike, hitEs );
    ...
    write_to_disk(results, num_spike, time_spike, hitEs, NUM_SAMPLES);

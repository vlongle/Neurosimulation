...
void get_random(double low, double high, int N){
    int numThreads;
#pragma omp parallel
    {
        ...
        for(int i=0; i < N; i++){
            ...
            double rand_float = low+(high-low)*u(mt);
...
void generate_random_connections(Vector<Parameters> *pi, double SEE_low, double SEE_high,
...
//    double SII_high = -3.0;
        ...
        generate_random_connections(&pi, SEE_low, SEE_high, SIE_low,
...
int main()
{
    ...
    char* file_name_machine = "june_data_unrestricted.txt";
    char* file_name_human = "june_data_unrestricted_human.txt";
